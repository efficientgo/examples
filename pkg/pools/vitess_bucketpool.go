...
// Taken from https://github.com/vitessio/vitess/blob/be593bbe23f5d28f54321bb9803f823faacc7cad/go/bucketpool/bucketpool.go
...
// BucketedPool is actually multiple pools which store buffers of specific size.
...
// NewBucketedPool returns BucketedPool which has buckets from minSize to maxSize.
// Buckets increase with the power of two, i.e with multiplier 2: [2b, 4b, 16b, ... , 1024b]
// Last pool will always be capped to maxSize.
func NewBucketedPool(minSize, maxSize int) *BucketedPool {
	if maxSize < minSize {
...
func (p *BucketedPool) findPool(size int) *sizedPool {
	if size > p.maxSize {
		return nil
	}
...
func (p *BucketedPool) Put(b *[]byte) {
	sp := p.findPool(cap(*b))
	if sp == nil {
		return
	}
