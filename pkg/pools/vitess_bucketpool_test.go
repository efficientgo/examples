...
func TestBucketedPool(t *testing.T) {
	maxSize := 16384
	pool := NewBucketedPool(1024, maxSize)
...
func TestBucketedPoolOneSize(t *testing.T) {
	maxSize := 1024
	pool := NewBucketedPool(1024, maxSize)
	if pool.maxSize != maxSize {
		t.Fatalf("Invalid max pool size: %d, expected %d", pool.maxSize, maxSize)
...
func TestBucketedPoolTwoSizeNotMultiplier(t *testing.T) {
	maxSize := 2000
	pool := NewBucketedPool(1024, maxSize)
	if pool.maxSize != maxSize {
		t.Fatalf("Invalid max pool size: %d, expected %d", pool.maxSize, maxSize)
...
func TestBucketedPoolWeirdMaxSize(t *testing.T) {
	maxSize := 15000
	pool := NewBucketedPool(1024, maxSize)
	if pool.maxSize != maxSize {
		t.Fatalf("Invalid max pool size: %d, expected %d", pool.maxSize, maxSize)
...
func TestFuzz(t *testing.T) {
	...
	for i := 0; i < 20000; i++ {
		...
		p := NewBucketedPool(minSize, maxSize)
...
func BenchmarkBucketedPool(b *testing.B) {
	pool := NewBucketedPool(2, 16384)
	b.SetParallelism(16)
	b.ResetTimer()
...
func BenchmarkBucketedPoolGet(b *testing.B) {
	pool := NewBucketedPool(2, 16384)
	b.SetParallelism(16)
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
